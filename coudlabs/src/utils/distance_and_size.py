...
def distsize_II(data_ref, data_other, real_size_ref, focal_length):
    frame_numbers_ref = set(int(row[0]) for row in data_ref)
    frame_numbers_other = set(int(row[0]) for row in data_other)
    image_size_other_prev = None
    image_size_ref_prev = None
    ...
    for frame_number in common_frame_numbers:
        ...
        if matching_rows_other:  # Check if matching_rows_other is not empty
            for ref, other in zip(matching_rows_ref, matching_rows_other):
                image_size_ref = float(ref[4])
                image_size_other = float(other[4])

                distance_ref = focal_length * real_size_ref / image_size_ref
                distance_ref_list.append(distance_ref)

                if image_size_ref_prev is not None and image_size_other_prev is not None:
                    image_size_diff_ref = 1 / image_size_ref_prev - 1 / image_size_ref
                    image_size_diff_other = 1 / image_size_other_prev - 1 / image_size_other

                    if image_size_diff_other != 0:
                        real_size_other = real_size_ref * image_size_diff_ref / image_size_diff_other
                        real_size_other_list.append(real_size_other)
                        distance_other = focal_length * real_size_other / image_size_other
                        distance_other_list.append(distance_other)
                    else:
                        real_size_other_list.append(None)
                        distance_other_list.append(None)
                
                image_size_ref_prev = image_size_ref
                image_size_other_prev = image_size_other
        else:  # Append None values to maintain indices
            distance_ref_list.append(None)
            distance_other_list.append(None)
            real_size_other_list.append(None)
